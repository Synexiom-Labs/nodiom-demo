...
const AGENTS = ['planner', 'researcher', 'reviewer', 'summarizer', 'scheduler'];

console.log('\n── DEMO 4: five agents, one document ──────────────────────\n');
console.log(`  Endpoint: ${API_URL}`);
console.log(`  Key:      ${USING_SANDBOX ? 'shared public sandbox' : 'your own key'}\n`);

/** Which agents' tasks actually made it into the document. */
...
await runDemo(async () => {
  // ─────────────────────────────────────────────────────────────────────────
  // Round A — full-document read/modify/write, the way an agent does it
  // without structural tools. Each agent's cycle spans two round trips.
  // ─────────────────────────────────────────────────────────────────────────
    ...
    await Promise.all(
      AGENTS.map(async (agent) => {
        ...
        await callTool('nodiom_write', {
          doc_id: docA,
          selector: '# Project Atlas',
          new_content: body,
        });
      }),
    );

    const lost = report(await survivors(docA));
    if (lost.length > 0) {
